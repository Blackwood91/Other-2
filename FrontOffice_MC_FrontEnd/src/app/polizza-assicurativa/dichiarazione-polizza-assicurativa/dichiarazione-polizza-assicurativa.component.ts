...
export class DichiarazionePolizzaAssicurativaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      ...
      this.serviceME
        .getPolizzaAssicurativaDOMODMP("richiesta/getPolizzaAssicurativa", this.idRichiesta)
        .subscribe({
          next: (res: any) => {
            if (res === null) {
  ...
  loadIsConvalidato() {
    ...
    this.serviceME
      .getStatusPolizza("status/getModuloIsConvalidato", params)
      .subscribe({
        next: (res: any) => {
  ...
  checkDate() {
    ...
    if (dateA > dataActual && differenzaGiorni >= giorniAnnoDatafine) {
  ...
  checkMassimaleAss() {
    const cifra = this.massimaleAssicurato
      .replace(/\./g, "")
      .replace(/,/g, ".");
  ...
  openPdfFile() {
    ...
    this.serviceME
      .getAnteprimaFilePolizzaAss("pdf/getAnteprimaFilePolizzaAss", params)
      .subscribe({
        next: (res: any) => {
  ...
  sendDichiarazionePolizzaAssicurativa() {
    if(!this.checkPrVisioneRinP) {
      this.sharedService.onMessage("error", "Per proseguire, è necessario selezionare la casella della presa visione della normativa");
      return;
    }
    
    // CHECK SULLA DISTANZA DELLA DATA 
    this.checkDate();
    
    if (this.dataStipulaPoliz != "" && this.scadenzaPoliza != "" &&
        this.massimaleAssicurato != "" && this.compagniaAssicuratrice.length > 0) {
    ...
    const polizzaAssicurativaDto: {
      idRichiesta: number;
      compagniaAssicuratrice: string;
      massimaleAssicurato: number;
      dataStipulaPoliz: string;
      scadenzaPoliza: string;
    } = {
      idRichiesta: this.idRichiesta,
      compagniaAssicuratrice: this.compagniaAssicuratrice,
      massimaleAssicurato: this.formatNumberDecimal(),
      dataStipulaPoliz: this.dataStipulaPoliz,
      scadenzaPoliza: this.scadenzaPoliza,
    };

    this.serviceME
      .savePolizzaAssicurativa(
        "richiesta/saveDichiarazionePolizzaAss",
        polizzaAssicurativaDto
      )
      .subscribe({
        next: (res: any) => {
  ...
  convalidazione() {
    ...
    const params = new HttpParams().set("idRichiesta", this.idRichiesta);
    this.serviceME
      .convalidazionePolizzaAssicurativa("status/convalidazioneDichiarazionePolizzaAss", params)
      .subscribe({
        next: (res: any) => {
          this.loadIsConvalidato();
          this.sharedService.onUpdateMenu();
