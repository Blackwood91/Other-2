...
export class RiepilogoSchedaPrestatoreServizioComponent implements OnInit {
  ...
  openModal(idRichiesta: number, idAnagrafica: number) {
    ...
    const buttonActiveModal = document.getElementById("activeModalSchedaRiepilogoPrestSer");
  ...
  openPdfFile() {
    ...
    this.serviceME.getFileRappresentante('pdf/getFilePrestatore', params)
      .subscribe({ 
        next: (res: any) => { 
          var file = new Blob([this.convertiStringaBlobAFile(res[0].file)], { type: 'application/pdf' });
