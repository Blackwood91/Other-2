...
export class InserimentoSchedaPrestatoreServizioComponent implements OnInit {
  ...
  saveAnagrafica() {
    if(this.finalCheck() === true) {
      let datiAnagrafica: {
        idAnagrafica: number,
        idTitoloAnagrafica: number, sesso: string, cognome: string, nome: string, dataNascita: string,
        poDataAssunzione: string, statoNascita: string, idComuneNascita: number, codiceFiscale: string, 
        cittadinanza: string, poTipoRappOdm: string, comuneNascitaEstero: string, idRichiesta: number
      } = {
        idAnagrafica: 0,
        idTitoloAnagrafica: this.idTitolo, sesso: this.sesso, cognome: this.cognome,
        nome: this.nome, dataNascita: this.dataNascita, poDataAssunzione: this.poDataAssunzione, 
        statoNascita: this.statoNascita, idComuneNascita: this.idComuneNascita,
        codiceFiscale: this.codiceFiscaleSS, cittadinanza: this.cittadinanza, 
      ...
      formData.append('anagraficaDto', new Blob([JSON.stringify(datiAnagrafica)], { type: 'application/json' }));
      ...
      else if(this.fileDocumento == null/* && this.existFileDocumento != true*/) {
      ...
      this.serviceME.saveAnagraficaPrestatore('anagrafica/saveAnagraficaPrestatore', formData)
        .subscribe({
          next: (res: any) => {
