...
export class SediComponent implements OnInit {
  ...
  @ViewChild(DettaglioSedeComponent) dettaglioSedeComponent!: DettaglioSedeComponent;
  ...
  openModalSaveSedeUpdate(idSede: number, idComune: number, nomeComune: string, siglaProvincia: string) {
    this.saveSedeComponent.openModalUpdate(this.idRichiesta, idSede, idComune, nomeComune, siglaProvincia);
  }
  ...
  openModalConfirmMessage(idSede: number, sedeLegale: string, infoSede: string) {
    if (sedeLegale == "1") {
      this.sharedService.onMessage('error', "Non si può proseguire con la cancellazione perchè la sede è legale");
      return;
    }
    
    let message = "Si vuole confermare la cancellazione della sede sita in: " + infoSede + "?";
  ...
  getModalChildrenConfirm(event: any) {
    if (event.message == "confermato") {
      const params = new HttpParams()
        .set('idSede', event.idRitorno)

      this.serviceME.deleteSedeOperativa('sede/deleteSedeOperativa', params)
        .subscribe({
          next: (res: any) => {
            this.isModifica = false;
            this.loadTableSO();
          ...
          error: (error: any) => {
            this.sharedService.onMessage('error', "La cancellazione non è andata a buon fine");
          }
        })
    }
  ...
  convalidazione() {
    ...
    this.serviceME
      .validazioneSedi('status/sediAttoRiepilogativo', params)
      .subscribe({
        next: (res: any) => {
          this.isModifica = false;
          this.sharedService.onUpdateMenu();
  ...
  viewSede(idSede: number, idComune: number, nomeComune: string, siglaProvincia: string) {
    this.dettaglioSedeComponent.openModal(this.idRichiesta, idSede, idComune, nomeComune, siglaProvincia);
  }
  ...
  validazione() {
    ...
    this.serviceME
      .validaSediAttoRiepilogativo('status/validaSediAttoRiepilogativo', params)
      .subscribe({
        next: (res: any) => {
          this.sharedService.onUpdateMenu();
          this.loadTableSO();
  ...
  annullazione() {
    ...
    this.serviceME
      .annullaSediAttoRiepilogativo('status/annullaSediAttoRiepilogativo', params)
      .subscribe({
        next: (res: any) => {
          this.sharedService.onUpdateMenu();
          this.loadTableSO();
