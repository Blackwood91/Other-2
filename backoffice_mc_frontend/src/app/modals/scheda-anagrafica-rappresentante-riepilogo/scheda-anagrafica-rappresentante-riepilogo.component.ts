...
@Component({
  selector: 'app-scheda-anagrafica-rappresentante-riepilogo',
  templateUrl: './scheda-anagrafica-rappresentante-riepilogo.component.html',
  styleUrls: ['./scheda-anagrafica-rappresentante-riepilogo.component.css']
})

export class SchedaAnagraficaRappresentanteRiepilogoComponent implements OnInit {
  ...
  openModal(idRichiesta: number, idAnagrafica: number, updateAnagraficaRapLegale: boolean) {
    ...
    this.serviceME
      ...
      .subscribe((res: any) => {
        ...
        if (res != null && res[1] != null) {
          this.fileQualificaMed = res[1].file;
        }
  ...
  getFileQualifica() {
    var file = new Blob([this.convertiStringaBlobAFile(this.fileQualificaMed)], { type: 'application/pdf' });
    var fileURL = URL.createObjectURL(file);
    window.open(fileURL, '_blank');
  }
