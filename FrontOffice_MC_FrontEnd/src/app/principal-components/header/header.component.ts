...
export class HeaderComponent implements OnInit {
  ...
  constructor(
    ...
    private mediazioneService: MediazioneService,
  ...
  logoutIamg() {
    ...
    if (this.accessibilita.getCookieValue("isAdn") == "false") {
      window.location.href = encodeURI(
        "https://auth03coll.giustizia.it/b2cmingiustiziaspidcoll.onmicrosoft.com/b2c_1a_signin_aad_spid/oauth2/v2.0/logout?clientId=" +
          this.env.uriConfig.clientId +
          "&post_logout_redirect_uri=" +
          this.env.uriConfig.logout_redirect_uri
      );
    } else {
      ...
      const finestraDisconessione = window.open(
        "https://login.microsoftonline.com/common/oauth2/v2.0/logout?clientId=" +
          this.env.uriConfig.clientId,
