...
export class SchedaAnagraficaMediatoreComponent implements OnInit {
  ...
  medNumeroOrganismiDisp: number = 0;
  ...
  saveAnagraficaGen() {
    if (this.finalCheck() === true) {
      let anagrafica: {
        ...
        medUniversita: string;
        idRichiesta: number;
        medDataOrdineCollegioProfess: string;
      } = {
        idAnagrafica: 0,
        ...
        medDataOrdineCollegioProfess: this.dataOrdColProf == null || this.dataOrdColProf == "Invalid date" ? "" : this.dataOrdColProf,
      };
      ...
      this.sharedService.onMessage("error", "Per proseguire al salvataggio è necessario avere tutti i campi obbligatori completati e validi");
    }
  ...
  checkPiva() {
    ...
    if (partitaIvaRegex.test(this.piva)) {
      this.validPiva = true;
      return true;
    } else {
  ...
  checkIndirizzoEmail() {
    ...
    if (emailRegex.test(this.indirizzoEmail)) {
      this.validIndirizzoEmail = true;
      return true;
    } else {
  ...
  checkIndirizzoPec() {
    ...
    if (emailRegex.test(this.indirizzoPec)) {
      this.validIndirizzoPec = true;
      return true;
    } else {
