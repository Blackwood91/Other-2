...
export class AlboMediatoriComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.onInitLoad();
    this.serviceME
      .getAllMediatoriPerAlbo("alboMediatori/getAllMediatoriIscrittiPaged", {})
      .subscribe({
        next: (res: any) => {
          this.totalRows = res.result;
        },
      });
  }
  ...
  cambiaPagina(index: number) {
    ...
    this.serviceME
      .getAllMediatoriPerAlbo("alboMediatori/getAllMediatoriIscrittiPaged", params)
      .subscribe((res: any) => {
        this.tableResult = res.result;
      });
  ...
  public getMediatori() {
    return this.tableResult.filter(
      (i) =>
        i.idTipoAnagrafica == "4" ||
        i.idTipoAnagrafica == "5" ||
        i.idTipoAnagrafica == "6"
    );
  }

  public getRows() {
    return this.totalRows.filter(
      (i) =>
        i.idTipoAnagrafica == "4" ||
        i.idTipoAnagrafica == "5" ||
        i.idTipoAnagrafica == "6"
