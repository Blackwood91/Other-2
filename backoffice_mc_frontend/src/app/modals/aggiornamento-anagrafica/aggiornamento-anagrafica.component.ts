...
export class AggiornamentoAnagraficaComponent implements OnInit {  
  ...
  openModal(idAnagrafica: number) {
    ...
    const buttonActiveModal = document.getElementById("activeModalUpdateAnagrafica");
  ...
  sendAnagrafica(idAnagrafica: number) {
    ...
    if (this.finalCheck() === true) {
      let datiAnagrafica: {
        idAnagrafica: number, idTitoloAnagrafica: number, sesso: string, cognome: string, nome: string, dataNascita: string | null,
        poDataAssunzione: string | null, statoNascita: string, idComuneNascita: number, codiceFiscale: string,
        cittadinanza: string, poTipoRappOdm: string, comuneNascitaEstero: string
      } = {
        idAnagrafica: this.idAnagrafica, idTitoloAnagrafica: this.idTitolo, sesso: this.sesso, cognome: this.cognome,
        nome: this.nome, dataNascita: this.dataNascita /*== undefined ? this.dataNascita : null*/,
        poDataAssunzione: this.poDataAssunzione /*== undefined ? this.poDataAssunzione : null*/,
        statoNascita: this.statoNascita, idComuneNascita: this.idComuneNascita,
        codiceFiscale: this.codiceFiscale, cittadinanza: this.cittadinanza,
        poTipoRappOdm: this.poTipoRappOdm, comuneNascitaEstero: this.comuneNascitaEstero
      }
      ...
      this.serviceME.updateAnagrafica('anagrafica/saveAnagrafica', datiAnagrafica)
        .subscribe({
          next: (res: any) => {
  ...
  finalCheck() {
    ...
    if (this.checkCittadinanza() === false)
      esito = false;
    if (this.checkRapporto() === false)
      esito = false;
    if (this.checkDataAssunzione() === false)
      esito = false;
