...
export class ElencoComponentiOrganoAmmComponent implements OnInit {
  ...
  @ViewChild(SchedaAnagraficaCompOrgAmComponent)
  schedaAnagraficaCompOrgAmComponent!: SchedaAnagraficaCompOrgAmComponent;
  @ViewChild(SchedaAnagraficaCompOrgAmRiepilogoComponent)
  schedaAnagraficaCompOrgAmRiepilogoComponent!: SchedaAnagraficaCompOrgAmRiepilogoComponent;
  ...
  openModalUpdateScheda(idAnagrafica: number) {
    this.schedaAnagraficaCompOrgAmComponent.openModal(
      this.idRichiesta,
      idAnagrafica
    );
  }
  ...
  openModalRiepilogoScheda(idAnagrafica: number) {
    this.schedaAnagraficaCompOrgAmRiepilogoComponent.openModal(
      this.idRichiesta,
      idAnagrafica,
  ...
  loadTable() {
    ...
    this.serviceME
      .getAllAnagraficaByIdRichiestaForEleCompOrgAm(
        "anagrafica/getAllAnagraficaByIdRichiestaForEleCompOrgAm",
        params
      )
      .subscribe((res: any) => {
  ...
  loadIsConvalidato() {
    ...
    this.serviceME
      .getStatusElenRapLegAndRespOrg("status/getStatusElenCompOrgAm", params)
      .subscribe({
        next: (res: any) => {
          this.isConvalidato = res.status === "c" ? true : false;
        },
        error: (error: any) => {
          this.sharedService.onMessage("error", error);
        },
  ...
  convalidazione() {
    ...
    this.serviceME
      .convalidazioneElencoCompOrgAmAndCompSoc(
        "status/convalidazioneElencoCompOrgAmAndCompSoc",
        params
      )
      .subscribe({
        next: (res: any) => {
        ...
        error: (error: any) => {
          ...
          this.extraInfoComponent.openModal(
            "Impossibile proseguire con la convalidazione:",
            "Per proseguire è necessario convalidare l'autocertificazione di requisiti di onorabilità di tutti i componenti dell'organo di amministrazione"
          );
        },
      });
  }
  ...
  openPdfFile() {
    ...
    this.serviceME
      .getAnteprimaFileSchedeMediatoriCompOrgAm(
        "pdf/getAnteprimaFileSchedeMediatoriCompOrgAm",
        params
      )
      .subscribe({
        next: (res: any) => {
