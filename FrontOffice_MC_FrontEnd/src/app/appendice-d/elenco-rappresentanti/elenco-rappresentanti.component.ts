...
export class ElencoRappresentantiComponent implements OnInit {
  ...
  totalPage = 0;
  totalResult = 0;
  @ViewChild(SchedaAnagraficaRappresentantiComponent)
  schedaAnagraficaRappresentantiComponent!: SchedaAnagraficaRappresentantiComponent;
  @ViewChild(SchedaAnagraficaRappresentanteRiepilogoComponent)
  schedaAnagraficaRappresentanteRiepilogoComponent!: SchedaAnagraficaRappresentanteRiepilogoComponent;
  ...
  openModalUpdateScheda(idAnagrafica: number) {
    this.schedaAnagraficaRappresentantiComponent.openModal(
      this.idRichiesta,
      idAnagrafica,
      this.tableResult.length > 1 ? false : true,
      this.rapLegCloneRespOperativa != null &&
        idAnagrafica == this.rapLegCloneRespOperativa.idAnagrafica
    );
  }

  openModalRiepilogoScheda(idAnagrafica: number) {
    this.schedaAnagraficaRappresentanteRiepilogoComponent.openModal(
      this.idRichiesta,
      idAnagrafica,
      this.rapLegCloneRespOperativa != null &&
        idAnagrafica == this.rapLegCloneRespOperativa.idAnagrafica
  ...
  loadTableSO() {
    ...
    this.serviceME
      .getAllAnagraficaByIdRichiestaForRapLegAndRespOrg(
        "anagrafica/getAllAnagraficaByIdRichiestaForRapLegAndRespOrg",
        params
      )
      .subscribe((res: any) => {
        for (let i = 0; i < res.result.length; i++) {
          ...
          if (res.result[i].idQualifica == 1) {
            ...
            this.serviceME
              .getAnagraficaCloneRespOrg(
                "anagrafica/getAnagraficaCloneRespOrg",
                params
              )
              .subscribe((res2: any) => {
                if (res2 != null) {
                  ...
                  for (let y = 0; y < res.result.length; y++) {
                    if (
                      res.result[i].idAnagrafica ==
                        res.result[y].idAnagrafica &&
  ...
  loadIsConvalidato() {
    ...
    this.serviceME
      .getStatusElenRapLegAndRespOrg(
        "status/getStatusElenRapLegAndRespOrg",
        params
      )
      .subscribe({
        next: (res: any) => {
  ...
  convalidazione() {
    ...
    this.serviceME
      .convalidazioneElencoRapLegAndRespOrg(
        "status/convalidazioneElencoRapLegAndRespOrg",
        params
      )
      .subscribe({
        next: (res: any) => {
  ...
  openPdfFile() {
    ...
    this.serviceME
      .getAnteprimaFileSchedeRapLegRespOrg(
        "pdf/getAnteprimaFileSchedeRapLegRespOrg",
        params
      )
      .subscribe({
        next: (res: any) => {
