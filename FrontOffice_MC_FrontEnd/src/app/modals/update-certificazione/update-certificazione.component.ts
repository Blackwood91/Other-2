...
export class UpdateCertificazioneComponent implements OnInit {
  ...
  openModal(
    titolo: string,
    idStatoModuloFiglio: number,
    idRichiesta: number,
    idCertificazioneLingua: number,
    dataCertificazione: string,
    enteCertificatore: string
  ) {
    this.titolo = titolo;
    ...
    this.dataCertificazione = !!dataCertificazione
      ? moment(dataCertificazione).format("YYYY-MM-DD")
      : "";
  ...
  sendCertificazione() {
    const fileAllegatoAppendiciDto: {
      ...
      dataCertificazione: string;
      enteCertificatore: string;
      file: any /*, idRiferimento: number*/;
      nomeFile: string;
    } = {
      id: this.idStatoModuloFiglio,
      idCertificazioneLingua: this.idCertificazioneLingua,
      idRichiesta: this.idRichiesta,
      dataCertificazione: this.dataCertificazione,
      enteCertificatore: this.enteCertificatore,
      file: null /*, idRiferimento: 0*/,
      nomeFile: this.selectedFilePDF != null ? this.selectedFilePDF.name : "",
    };
    ...
    this.serviceME
      .saveFileAttoCostitutivoOdm("statoModuli/updateFileAppendici", formData)
      .subscribe({
        next: (res: any) => {
