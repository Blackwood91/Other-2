...
@Component({
  ...
  styleUrls: ["./lettura-pdg.component.css"],
})
export class LetturaPdgComponent implements OnInit {
  ...
  loadTable() {
    ...
    this.serviceME
      .getAllEmissionePdgOdmForTable(
        "emissionePdgOdm/getAllEmissionePdgOdmForTable",
        params
      )
      .subscribe((res: any) => {
  ...
  openPdfFile(idEmissionePdg: number) {
    const params = new HttpParams().set("idEmissionePdg", idEmissionePdg);

    this.serviceME.getFilePdg("emissionePdgOdm/getFilePdg", params).subscribe({
      next: (res: any) => {
