...
@Component({
  selector: 'app-autocertificazione-onorabilita',
  templateUrl: './autocertificazione-onorabilita.component.html',
  styleUrls: ['./autocertificazione-onorabilita.component.css']
})
export class AutocertificazioneOnorabilitaComponent implements OnInit {
  @Input()
  ...
  loadSelectResponsabili() {
    const params = new HttpParams()
      .set('idRichiesta', this.idRichiesta)

    this.serviceME.getAllRappresentantiAutocertificazioni('anagrafica/getAllRappresentantiAutocertificazioni', params)
      .subscribe((res: any) => {
        this.rappresentanti = res.list;
        this.anagraficaRapLegClone = res.anagraficaRapLegClone;        
      });
  }
  ...
  getTitolo(idTitolo: number) {
    let titolo = this.titoliAnagrafiche.find((obj: { idTitoloAnagrafiche: number; }) => { return obj.idTitoloAnagrafiche === idTitolo });
    if (titolo)
  ...
  openPdfFile() {
    ...
    this.serviceME.getAnteprimaFileAutocertificazioneReqOno('pdf/getAnteprimaFileAutocertificazioneReqOno', params)
      .subscribe({
        next: (res: any) => {
  ...
  loadOnorabilitaIsConvalidato() { 
    const params = new HttpParams()
      ...
      this.serviceME.getStatusRquisitiOnorabilita('status/getStatusReqOno', params)
        .subscribe({
          next: (res: any) => {
  ...
  validazione() {
    const params = new HttpParams()
      .set('idRichiesta', this.idRichiesta)
      .set('idAnagrafica', this.idAnagrafica)
      .set('isCloneAnagraficaRapLegale', (this.anagraficaRapLegClone != null && 
        this.anagraficaRapLegClone.codiceFiscale == this.codiceFiscale) ? true : false);

    this.serviceME
      .validazioneRequisitiOnorabilitaAppendici('status/validazioneRequisitiOnorabilita', params)
      .subscribe({
        next: (res: any) => {
        ...
        error: (error: any) => {
          this.sharedService.onMessage('error', "Non è stato possibile proseguire con la convalidazione");
        } 
      });
  }
  ...
  annullazione() {
    ...
    this.serviceME
      .annullaRequisitiOnorabilitaAppendici('status/annullaRequisitiOnorabilita', params)
      .subscribe({
        next: (res: any) => {
          this.sharedService.onUpdateMenu();
          this.loadOnorabilitaIsConvalidato();
        ...
        error: (error: any) => {
          this.sharedService.onMessage('error', "Non è stato possibile proseguire con l'annullamento");
        }
      });
  }
  ...
  loadAutocertificazione(idAnagrafica: any) {
    ...
    this.serviceME
    .getElencoAutocerficatiRequisitiOnorabilitApp('statoModuli/getElencoAutocerficatiRequisitiOnorabilitApp', params)
    .subscribe({
      next: (res: any) => {
