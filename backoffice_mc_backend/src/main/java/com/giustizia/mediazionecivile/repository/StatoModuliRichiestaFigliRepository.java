...
//	@Query("select case when completato = 1 then true else false end "
//			+ "from StatoModuliRichiestaFigli "
//			+ "where idModulo = :idModulo and idRichiesta = :idRichiesta")
//	boolean existsByCustomQuery(Long idModulo, Long idRichiesta);
	...
	Optional<StatoModuliRichiestaFigli> findFirstByIdModuloAndIdRichiestaAndIdRiferimentoOrderByDataInserimentoDesc(Long idModulo, Long idRichiesta,
			Long idRiferimento);
	
	Optional<StatoModuliRichiestaFigli> findByIdModuloAndIdRichiestaAndIdAnagrafica(Long idModulo, Long idRichiesta,
			Long idAnagrafica);
	
    @Query("SELECT s FROM StatoModuliRichiestaFigli s  WHERE s.idModulo = :idModulo " +
           "AND s.idRichiesta = :idRichiesta ORDER BY s.dataInserimento DESC")
     Optional<StatoModuliRichiestaFigli> findLatestByIdModuloAndIdRichiesta(
	...
	@Query("SELECT s.progressivoModulo, s.idModulo, s.completato, s.validato, s.annullato,"
		       + " m.descrizione, a.nome, a.cognome, se.indirizzo, se.numeroCivico, c.nomeComune, r.siglaProvincia,"
		       + " s.nomeAllegato"
		       + " FROM StatoModuliRichiestaFigli s "
		       + " JOIN Modulo m ON m.idModulo = s.idModulo "
		       + " LEFT JOIN AnagraficaOdm a ON a.idAnagrafica = s.idAnagrafica AND s.idRichiesta = :idRichiesta "
		       + " LEFT JOIN Sede se ON se.idSede = s.idRiferimento AND se.idRichiesta = :idRichiesta "
		       + " LEFT JOIN Comune c ON c.idCodComune = se.idComune "
		       + " LEFT JOIN RegioneProvince r ON r.codiceProvincia = c.codiceProvincia "
		       + " WHERE s.idRichiesta = :idRichiesta AND s.annullato = 1"
		       + " ORDER BY s.idModulo")
    List<Object[]> findAllForRichiestaIntegrazione(@Param("idRichiesta") Long idRichiesta);
	
	@Query("SELECT s FROM StatoModuliRichiestaFigli s WHERE s.id IN (SELECT MIN(s2.id) FROM StatoModuliRichiestaFigli s2"
			   + " WHERE s2.idRichiesta=:idRichiesta GROUP BY s2.idModulo) ORDER BY s.idModulo")
	...
	//data_certificazione, ente_certificatore
	@Query(value = "SELECT cls.id_anagrafica, cls.ID_CERTIFICAZIONE_LINGUA, cls.data_certificazione,"
			+ " cls.ente_certificatore, smrf.NOME_ALLEGATO, smrf.data_inserimento, smrf.ID_STATO_MODULI_RICHIESTA_FIGLI, "
			+ " smrf.completato, smrf.validato, smrf.annullato "
			...
			+ " GROUP BY id_anagrafica, ID_CERTIFICAZIONE_LINGUA) cls "
